...
Do not bake `VITE_HTTP_URL`, `VITE_WS_URL`, or a public origin into
the web bundle. Same-origin serving is the default: the browser talks to the host
it loaded.

## Secrets

Never put a live deploy token in git.

| Variable               | Where                                                                                                                                                        |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `T3CODE_SESSION_DATA_ISOLATION` | On by default. Pairings only see projects owned by their GitHub account (or that pairing session before GitHub OAuth). Set `0`/`false`/`off`/`no` to share one environment across every pairing. See [session-data-isolation.md](../internals/session-data-isolation.md). |
| `T3CODE_PUBLIC_URL`    | Public `https://` origin. Used as the pairing JWT audience. Same sources as the GitHub App secrets.                                                                         |
| `GITHUB_CLIENT_ID`     | GitHub App or OAuth App client ID (`Iv23…` for GitHub Apps). Placeholders and numeric App IDs are ignored. Use a **public** GitHub App (hosted: `coda-io-intel`). Private apps 404 for anyone except the owner. The app’s callback URL must include `https://www.iointel.dev/api/auth/github/callback`. Packed into `zerops.yml` by `pack.sh` or set in the Zerops service env UI. |
| `GITHUB_CLIENT_SECRET` | Client secret for the GitHub App. Same sources as `GITHUB_CLIENT_ID`.                                                                                        |
| `GITHUB_REDIRECT_URI`  | Exact public callback URL: `https://<stack-host>/api/auth/github/callback`. Register the same URL in the GitHub App. Sign-in starts at `GET /api/auth/github/start` and the callback issues the browser session. |
...
| `.env.example`          | Template for pack-time public URL and GitHub OAuth          |
