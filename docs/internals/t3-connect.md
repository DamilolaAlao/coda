...
Coda Connect uses one Clerk application for web, desktop, and mobile authentication. The relay verifies
...
Coda Connect is disabled in a fresh clone. To enable it for source builds against the production
...
T3CODE_PORT=13773 \
  "/Applications/Coda (Alpha).app/Contents/MacOS/Coda (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/Coda (Alpha).app"
codesign -d --entitlements :- "/Applications/Coda (Alpha).app"
```
...
Signed-in users manage Coda Connect under **Connections**. The settings sidebar also has dedicated
controls, rendered by `SettingsSidebarNav.tsx`: `T3ConnectSidebarSignIn` in the footer shows a
**Sign in to Coda Connect** button while signed out, and `T3ConnectSidebarAvatar` shows a Clerk
