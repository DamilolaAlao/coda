...
## Using It with Coda Connect

Coda Connect may offer to install the service during setup so the host stays reachable after you log
out. This is only an onboarding shortcut: the service and Coda Connect are managed separately.

Signing out of Coda Connect does not remove the service. Use `t3 service uninstall` when you no longer
