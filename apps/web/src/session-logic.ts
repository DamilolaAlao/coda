...
export interface WorkLogEntry {
  ...
  agentRole?: string;
  /**
   * Present on agent-spawn conversation rows: one per workflow run or per-turn
   * batch of direct spawns. The row renders named speakers plus a CTA whose
   * live status is derived from the agent panel model at render time; clicking
 ...
 * One conversation row per group (A1 design): named speakers plus a CTA.
 */
...
function collapseDerivedWorkLogEntries(
  ...
  // own turn splintered one batch into a stream of spawn conversation rows
