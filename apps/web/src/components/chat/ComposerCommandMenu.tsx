...
function groupCommandItems(
  ...
  if (triggerKind === "path") {
    ...
    const pathItems = items.filter((item) => item.type !== "agent");
    ...
    if (pathItems.length > 0) {
      groups.push({
        id: "files",
        label: agentItems.length > 0 ? "Files" : null,
        items: pathItems,
      });
    }
    return groups;
...
const ComposerCommandMenuItem = memo(function ComposerCommandMenuItem(props: {
  ...
  return (
    ...
    >
      ...
      {props.item.type === "agent" ? <BotIcon className="size-4 shrink-0 text-icon-muted" /> : null}
