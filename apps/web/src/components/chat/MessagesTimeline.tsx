...
import {
  ...
  formatAgentConversationPreview,
...
/**
 * Spawn conversation: one anchored thread per workflow run (or per-turn
 * direct-spawn batch). Speakers are a Slack-style preview of the fleet;
 * the Agents panel remains the full roster. Static dots, no animation.
 */
const AgentSpawnCtaRow = memo(function AgentSpawnCtaRow(props: { workEntry: TimelineWorkEntry }) {
  ...
  const { agents, visibleAgents, hiddenCount, agentCount, workflowName, live, failed, working } =
    conversation;
  ...
  const lead = formatSpawnConversationLead(live, agentCount);
  const status = live
    ? livePhase
  ...
  return (
    <button
      ...
      className="-mx-1 flex w-full flex-col rounded-md border border-border/60 bg-card/50 text-left transition hover:bg-accent/40"
      data-agent-conversation="true"
    >
      <span className="flex w-full items-center gap-2 px-2.5 py-1.5 text-[13px]">
      ...
      {visibleAgents.length > 0 ? (
        <span className="flex flex-col border-t border-border/50">
          {visibleAgents.map((agent) => (
            <span
              key={agent.id}
              className="flex min-w-0 items-start gap-2 px-2.5 py-1.5"
              data-agent-conversation-speaker={agent.id}
            >
              <span
                aria-hidden
                className={cn(
                  "mt-0.5 flex size-6 shrink-0 items-center justify-center rounded-md text-[10px] font-semibold",
                  agentAvatarToneClass(agent.id),
                )}
              >
                {agentDisplayInitials(agent.title)}
              </span>
              <span className="min-w-0 flex-1">
                ...
                <span className="mt-0.5 block truncate text-[12px] text-muted-foreground">
                  {formatAgentConversationPreview(agent)}
                </span>
              </span>
            </span>
          ))}
          {hiddenCount > 0 ? (
            <span className="px-2.5 pb-1.5 text-[11px] text-muted-foreground">
              +{hiddenCount} more in Agents
            </span>
          ) : null}
        </span>
      ) : null}
