...
export interface ChatComposerProps {
  ...
  /** Fleet roster for @agent mentions in the composer. */
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const composerMenuItems = useMemo<ComposerCommandItem[]>(() => {
    ...
    if (composerTrigger.kind === "path") {
      const providerLabel =
        ...
        selectedProvider;
      const primaryAgent = primaryThreadAgentMention(providerLabel);
      const panelAgents = mentionableAgentsFromPanel(agentPanelModel).filter(
        (agent) => agent.handle !== primaryAgent.handle,
      );
      const mentionable =
        panelAgents.length > 0
          ? [primaryAgent, ...panelAgents]
          : composerTrigger.query.trim().length > 0
            ? [primaryAgent]
            : [];
      const agentItems = searchMentionableAgents(mentionable, composerTrigger.query).map(
        (agent) =>
          ({
            id: `agent:${agent.id}`,
            type: "agent" as const,
            handle: agent.handle,
            label: `@${agent.handle}`,
            description: agent.id.startsWith("primary:")
              ? agent.description
              : `${agent.title} · ${agent.description}`,
          }) satisfies Extract<ComposerCommandItem, { type: "agent" }>,
      );
  ...
  const onSelectComposerItem = useCallback(
    (item: ComposerCommandItem) => {
      ...
      if (!trigger) return;
      if (item.type === "agent") {
        const replacement = `${serializeComposerMentionPath(item.handle)} `;
