...
export const ModelPickerContent = memo(function ModelPickerContent(props: {
  ...
  const newBadgeInstanceIds = useMemo(() => {
    const ids = new Set<ProviderInstanceId>();
    for (const option of PROVIDER_OPTIONS) {
      if (option.pickerSidebarBadge !== "new") continue;
      const instanceId = defaultInstanceIdForDriver(option.value);
      if (sidebarInstanceEntries.some((entry) => entry.instanceId === instanceId)) {
        ids.add(instanceId);
      }
    }
    return ids;
      ...
      >
        ...
        {showSidebar && (
          <ModelPickerSidebar
            ...
            showFavorites
            newBadgeInstanceIds={newBadgeInstanceIds}
