...
import {
  ...
  hostedCanonicalRedirectLayer,
} from "./http.ts";
import { fixPath } from "./os-jank.ts";
import { websocketRpcRouteLayer } from "./ws.ts";
import * as ExternalLauncher from "./process/externalLauncher.ts";
import { httpClientHttpApiLayer } from "./httpClient/http.ts";
...
export const makeRoutesLayer = Layer.mergeAll(
  Layer.mergeAll(
    HttpApiBuilder.layer(EnvironmentHttpApi, { openapiPath: "/openapi.json" }).pipe(
      Layer.provide(authHttpApiLayer),
      Layer.provide(connectHttpApiLayer),
      Layer.provide(orchestrationHttpApiLayer),
      Layer.provide(pullRequestHttpApiLayer),
      Layer.provide(httpClientHttpApiLayer),
  ...
  Layer.provide(hostedCanonicalRedirectLayer),
);
        ...
        yield* forkParked(
          Effect.gen(function* () {
            ...
            yield* reconcileDesiredCloudLink(`http://127.0.0.1:${address.port}`).pipe(
              ...
              Effect.tap(() => Effect.logInfo("Coda Connect desired link reconciled on startup")),
              Effect.catch((cause) =>
                Effect.logWarning("Failed to reconcile Coda Connect desired link on startup", {
                  cause,
                }),
              ),
            );
