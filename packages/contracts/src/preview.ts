 ...
 *
 * Web clients render an iframe; desktop uses Chromium `<webview>`. The server
 * tracks per-thread tab metadata so it survives reconnects. The client reports
 * navigation back via these RPCs; the server fans events to subscribers.
 *
 * @module Preview
 */
